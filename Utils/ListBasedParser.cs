...
class ListBasedParser
{
    public static List<ListBasedOrderDependency> parseListBased(string filename)
    {
        ...
        foreach (var line in File.ReadAllLines(filename))
        {
            ...
            if (match.Success)
            {
                var lhs = match.Groups[1].Value.Split(",").Select(x => OrderSpecificationParser.parseOS(x)).ToList();
                var rhs = match.Groups[2].Value.Split(",").Select(x => OrderSpecificationParser.parseOS(x)).ToList();
